"""
Controlled A/B: does inverting the viridis colormap beat the raw G channel?

Only the INPUT representation differs between the two arms. Model architecture,
weight init, data split, data order, augmentation RNG, optimizer, schedule and
seed are all held identical. Single 80/20 stratified split for speed.
"""
import time
import numpy as np
import pandas as pd
...
EPOCHS = 22
BATCH = 128
CACHE = Path('/tmp/ab_inputs.pt')

device = torch.device('mps' if torch.backends.mps.is_available() else 'cpu')
print('device:', device)


# ---------- build both input representations (cached) ----------
def build_inputs():
    if CACHE.exists():
        b = torch.load(CACHE)
        print('loaded cached inputs')
        return b['XG'], b['XI'], b['y']

    vir = (colormaps['viridis'](np.linspace(0, 1, 256))[:, :3] * 255).astype(np.float32)
    QB = 64
    grid = (np.arange(QB) * 255 / (QB - 1)).astype(np.float32)
    rr, gg, bb = np.meshgrid(grid, grid, grid, indexing='ij')
    q = np.stack([rr, gg, bb], -1).reshape(-1, 3)
    idx = np.empty(q.shape[0], np.int32)
    for s in range(0, q.shape[0], 65536):
        c = q[s:s+65536]
        idx[s:s+65536] = (((c[:, None, :] - vir[None]) ** 2).sum(-1)).argmin(1)
    inv_lut = (idx / 255.0).astype(np.float32).reshape(QB, QB, QB)

    df = pd.read_csv(DATA / 'train.csv')
    N = len(df)
    XG = torch.empty(N, 1, IMG_H, IMG_W)
    XI = torch.empty(N, 1, IMG_H, IMG_W)
    for i, row in enumerate(df.itertuples(index=False)):
        arr = np.array(Image.open(DATA / 'train' / row.id))[:, :, :3].astype(np.int32)
        g = (arr[:, :, 1] / 255.0).astype(np.float32)                 # raw G channel
        qd = arr * (QB - 1) // 255
        inv = inv_lut[qd[..., 0], qd[..., 1], qd[..., 2]]             # inverted magnitude
        for X, im in ((XG, g), (XI, inv)):
    ...
    torch.save({'XG': XG, 'XI': XI, 'y': y}, CACHE)
    return XG, XI, y


# ---------- compact model (identical for both arms) ----------
class SE(nn.Module):
...
class AugDS(Dataset):
    """Light, valid augmentation; deterministic given global RNG seed + access order."""
...
def run_arm(name, X, y, tr, va):
    ...
    for ep in range(EPOCHS):
        ...
        acc = 100 * correct / len(Xva)
        best = max(best, acc)
        print(f'  [{name}] ep {ep+1:2d}/{EPOCHS}  val {acc:5.2f}  (best {best:5.2f})  mean={mean:.3f} std={std:.3f}', flush=True)
    return best


def main():
    XG, XI, y = build_inputs()
    print(f'dataset: {len(y)} imgs  | G mean/std {XG.mean():.3f}/{XG.std():.3f}'
          f' | INV mean/std {XI.mean():.3f}/{XI.std():.3f}')
    idx = np.arange(len(y))
    ...
    print('\n=== ARM B: viridis-inverted magnitude ===')
    bi = run_arm('INV', XI, y, tr, va)

    print('\n==================== A/B RESULT ====================')
    print(f'G channel        best val: {bg:.2f}%')
    print(f'viridis-inverted best val: {bi:.2f}%')
    print(f'delta (INV - G):           {bi - bg:+.2f} pp')
