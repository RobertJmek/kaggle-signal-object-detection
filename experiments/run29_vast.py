"""
Run 29 — count-aware EffNet, NATIVE ASPECT (384x165) + CAPACITY width=1.3, for a SINGLE CUDA GPU (vast.ai).

Lineage: run27_vast.py (Run 27 square 256, w1.1; Run 28 native 384x165, w1.1 -> Kaggle 0.78836). Run 29
stacks the 2nd confirmed structural lever: width 1.1 -> 1.3 (capacity A/B +3.45pp @native aspect, README
Exp 42; saturates at w1.3 — w1.6 regressed). Default N_ROUNDS=1 (Run 28's R2 added nothing). Identical
model/aug/training/pseudo-label logic; Kaggle creds via env or ~/.kaggle/kaggle.json, outputs to OUT_DIR,
single-GPU (no DataParallel), cudnn.benchmark on for Ampere. Offset is +4 (native), so Kaggle ~= CV + 4.
...
Run (survives disconnect):
    tmux new -s run29
    DATA_PATH=/root/comp/data OUT_DIR=./run29_out N_ROUNDS=1 python run29_vast.py 2>&1 | tee run29.log
    # detach: Ctrl-b then d ;  reattach: tmux attach -t run29
...
# ----------------------------- env / device -----------------------------
OUT_DIR = Path(os.environ.get('OUT_DIR', './run29_out')); OUT_DIR.mkdir(parents=True, exist_ok=True)
N_ROUNDS = int(os.environ.get('N_ROUNDS', '1'))   # 0 = 3-fold+TTA only. 1 = +pseudo R1 (Run 28 R1 +0.44; R2 saturated).
...
CONFIG = dict(epochs=58, batch_size=48, max_lr=1.0e-3, weight_decay=1e-3,
              ...
              width=1.3, depth_mult=1.2, ema_reset_epoch=3, pct_start=0.2)  # w1.1->1.3: capacity A/B +3.45pp @native (README Exp 42); w1.3 is the optimum (w1.6 regressed)
