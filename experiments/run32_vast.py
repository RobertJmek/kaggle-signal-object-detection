"""
Run 32 — ORDINAL (CORN) HEAD (single CUDA GPU). The label is an object COUNT (1-5, ordinal), not a nominal
class. Replace 5-way softmax+CE with a CORN ordinal head. First count-aware *representation* lever. Shot at 0.80.

Context: ensemble-member count is CONFIRMED but SATURATING. Holding the recipe fixed and varying only #members
gives a clean monotonic Kaggle series — Run 30 (2-fold) 0.78763 < Run 28 (3-fold) 0.78836 < Run 31 (4-fold)
0.78981 = NEW BEST. So member count genuinely transfers, but ~+0.001/member with diminishing returns: it will
NOT reach 0.80 alone (~7 more members for +1pp). Plateau is 0.790. Every *modeling* lever since Run 28
(resolution, capacity, multi-scale TTA) was flat or harmful on test. The only un-falsified category left is
COUNT-AWARE REPRESENTATION (memory: ensemble-members-confirmed-saturating, label-is-object-count).

Run 32 keeps the entire confirmed Run 31 recipe (native 448x192, w1.3, 4-fold/4-members, translation-TTA-only,
pseudo R1, guarded early stop) and changes ONE thing — the head:
  * CORN ordinal head: the final Linear outputs 4 threshold logits f_t = P(y > t | y > t-1), t=0..3, instead
    of 5 class logits. Trained with the CORN conditional-BCE loss (task t uses only samples with y >= t;
    binary target 1[y > t]); label-smoothed targets (0.05/0.95) to mirror the old CE label_smoothing=0.1.
  * `corn_probs()` converts the 4 logits to a proper 5-way categorical via cumulative products
    (P(y>t)=prod_{i<=t} sigmoid; P(y=k) = P(y>k-1) - P(y>k)). So EVERY downstream path — per-fold eval,
    ensembling, pseudo-label confidence, translation TTA — is byte-for-byte the softmax pipeline with
    `torch.softmax(logits)` swapped for `corn_probs(logits)`. argmax of the averaged categorical = predicted count.
  * Hypothesis: on an ordinal target where errors are mostly ±1, a "≥k" head respects the ordering and should
    lift EXACT-count accuracy by ~+1-3pp vs softmax (which treats counts 1 and 5 as equidistant).

GATEABLE ON CV (the key advantage): Run 32 uses the SAME 4-fold scheme as Run 31, so its 4-fold CV is DIRECTLY
comparable to Run 31's 4-fold CV (no fold-scheme confound — cf. Run 30 lesson). Promote/submit only if Run 32's
4-fold CV beats Run 31's 4-fold CV by >= +0.5pp. If it ties or regresses, the ordinal head doesn't help here;
keep softmax and pivot to the oriented-stem lever. (Still also sanity-check by Kaggle — but CV decides first.)

⚠️ TIME BUDGET (10h cap). 4-fold + pseudo R1 = 8 fold-trainings (same as Run 31; CORN head is the same FLOPs).
GUARDED EARLY STOP is ON (ES_PATIENCE=6, ES_GUARD=0.65): each fold stops once val plateaus in the anneal tail
WITHOUT truncating OneCycle's productive descent — best_state is always kept. WATCH THE FIRST FOLD'S WALL TIME
(printed): if 8 folds project > 10h, set EPOCHS=45 (recompresses the schedule) or N_ROUNDS=0 (drop pseudo,
~halves it). All knobs are env vars: TRAIN_FOLDS, N_SPLITS, EPOCHS, N_ROUNDS, ES_PATIENCE (0=off), ES_GUARD.
...
Run (survives disconnect):
    tmux new -s run32
    DATA_PATH=/root/comp/data OUT_DIR=./run32_out N_ROUNDS=1 python run32_vast.py 2>&1 | tee run32.log
    # detach: Ctrl-b then d ;  reattach: tmux attach -t run32
...
# ----------------------------- env / device -----------------------------
OUT_DIR = Path(os.environ.get('OUT_DIR', './run32_out')); OUT_DIR.mkdir(parents=True, exist_ok=True)
...
class EffNet(nn.Module):
    ...
    def __init__(s, n_logits=4, dropout=0.3, drop_path=0.1, width=1.0, depth_mult=1.0):
        super().__init__()   # n_logits=4 -> CORN ordinal thresholds P(y>t), t=0..3 (NOT 5-way softmax)
        ...
        s.head = nn.Sequential(nn.Conv2d(cin, ch(1280), 1, bias=False), nn.BatchNorm2d(ch(1280)), nn.SiLU(True),
                               nn.AdaptiveAvgPool2d(1), nn.Flatten(), nn.Dropout(dropout), nn.Linear(ch(1280), n_logits))
        for m in s.modules():
...
# --------- CORN ordinal head: 4 threshold logits <-> 5-way categorical (label = object count, ordinal 1-5) ---------
# logits[:,t] = conditional logit of P(y > t | y > t-1), t=0..3 (y is 0-indexed count, 0..4).
def corn_probs(logits):     # [B,4] -> [B,5] proper categorical. Drop-in replacement for torch.softmax in every path.
    cond = torch.sigmoid(logits.float())            # P(y>t | y>t-1)
    cum = torch.cumprod(cond, dim=1)                 # P(y>t) = prod_{i<=t} sigmoid -> monotone non-increasing
    p = torch.empty(logits.size(0), 5, device=logits.device, dtype=cum.dtype)
    p[:, 0] = 1 - cum[:, 0]                           # P(y=0)
    p[:, 1:4] = cum[:, :3] - cum[:, 1:]               # P(y=k) = P(y>k-1) - P(y>k), k=1..3
    p[:, 4] = cum[:, 3]                               # P(y=4)
    return p.clamp_min_(0)                            # guard tiny negatives from fp error

def corn_loss(logits, y, ls=0.1):     # -> [B] per-sample mean BCE over that sample's active CORN tasks
    out = torch.zeros(y.size(0), device=logits.device); cnt = torch.zeros_like(out)
    for t in range(4):
        mask = (y >= t).float()                      # CORN: task t conditions on y>t-1 i.e. uses samples with y>=t
        bt = (y > t).float(); bt = bt * (1 - ls) + 0.5 * ls   # label-smoothed binary target (mirrors CE ls=0.1)
        out += F.binary_cross_entropy_with_logits(logits[:, t], bt, reduction='none') * mask
        cnt += mask
    return out / cnt.clamp_min(1)

_m = EffNet(width=1.3, depth_mult=1.2); _n = sum(p.numel() for p in _m.parameters())
print(f'EffNet B2-scale CORN-head params: {_n:,} ({_n/1e6:.2f}M)', flush=True); del _m
...
def evaluate_cache(model, idx):     # always evaluates on REAL train cache -> leakage-free CV
    ...
    with torch.autocast(device_type=device.type, dtype=torch.float16, enabled=use_amp):
        for s0 in range(0, len(idx), 256):
            ...
            correct += (corn_probs(model(x)).argmax(1).cpu() == YTR[bi]).sum().item()
...
def train_fold(tr_idx, va_idx, cfg, tag='fold0', seed=42, xext=None, yext=None, pw=0.5):
    ...
    core = EffNet(4, cfg['dropout'], cfg['drop_path'], width=cfg['width'], depth_mult=cfg['depth_mult']).to(device)
    model = nn.DataParallel(core) if NGPU > 1 else core
    ema = EMA(core, cfg['ema_decay'])   # CORN ordinal loss (corn_loss) replaces CrossEntropy; ls mirrored inside it
    ...
    for ep in range(cfg['epochs']):
        ...
        for s0 in range(0, steps * bs, bs):
            ...
            with torch.autocast(device_type=device.type, dtype=torch.float16, enabled=use_amp):
                out = model(x); loss = (corn_loss(out, yb, cfg['label_smoothing']) * wb).sum() / wb.sum()
            scaler.scale(loss).backward(); scaler.step(opt); scaler.update(); sched.step(); ema.update(core)
            tc += (corn_probs(out).argmax(1) == yb).sum().item(); tn += yb.size(0)
...
def predict_test_probs(states):
    ...
    for st in states:
        m = EffNet(4, CONFIG['dropout'], CONFIG['drop_path'], width=CONFIG['width'], depth_mult=CONFIG['depth_mult']).to(device)
        ...
        with torch.no_grad(), torch.autocast(device_type=device.type, dtype=torch.float16, enabled=use_amp):
            for s0 in range(0, len(test_df), 256):
                ...
                probs[s0:s0 + 256] += corn_probs(m(x)).cpu()
...
print(f'\nFINAL CV {best_cv:.2f}%  ({TRAIN_FOLDS}-fold CORN-head scheme) -> best round applied', flush=True)
print('GATE: this is the SAME 4-fold scheme as Run 31 (softmax head) -> CV is DIRECTLY COMPARABLE this time.', flush=True)
print('      Promote the CORN head only if this CV beats Run 31\'s 4-fold CV by >= +0.5pp. If it ties/regresses,', flush=True)
print('      the ordinal head does not help here -> keep softmax, pivot to the oriented-stem lever. Then confirm', flush=True)
print('      on Kaggle (the ordinal-head submission is in submission.csv, already 448 + translation-TTA-only).', flush=True)
