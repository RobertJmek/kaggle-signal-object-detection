"""
Run 30 — AGGRESSIVE FAST EXPERIMENT (single CUDA GPU). Stack the BASELINE-INDEPENDENT levers, gamble for 80.

Context: Run 28 (native 384x165, w1.1) = Kaggle 0.78836. Run 29 (w1.3) = CV 74.83 ~= 0.789 -> capacity did
NOT transfer to deployment scale (proxy lied; README Exp 42/43). So Run 30 drops width and stacks the levers
that DO transfer (regime changes) + free test-time gains, kept FAST via 2 folds:
  1. KEEP width 1.3: at 384 w1.3==w1.1 (tie), but raising resolution to 448 means COMPOUND SCALING (res &
     capacity scale together) should now let the extra width pay — untested interaction, w1.3 at worst ties.
  2. 2 folds of a 5-WAY split (each model trains on 80% data, only 2 models -> fast + accurate; NOT KFold-2).
  3. resolution 448x192 (push the resolution lever — like aspect, a baseline-independent regime change).
  4. MULTI-SCALE TTA {384,448,512} at inference (EffNet global-pool head accepts any size) -> the multi-res
     ensemble for FREE, replacing the dead arch-ensemble (Exp 42).
  5. aggressive pseudo: top-500/class, floor 0.50 (more transductive signal than Run 28's 350/0.55).
This is an EXPERIMENT — many changes at once to move the score, not a clean ablation. Offset ~+4 (native).
...
Run (survives disconnect):
    tmux new -s run30
    DATA_PATH=/root/comp/data OUT_DIR=./run30_out N_ROUNDS=1 python run30_vast.py 2>&1 | tee run30.log
    # detach: Ctrl-b then d ;  reattach: tmux attach -t run30
...
# ----------------------------- env / device -----------------------------
OUT_DIR = Path(os.environ.get('OUT_DIR', './run30_out')); OUT_DIR.mkdir(parents=True, exist_ok=True)
N_ROUNDS = int(os.environ.get('N_ROUNDS', '1'))   # 0 = folds+TTA only. 1 = +pseudo R1 (Run 28 R1 +0.44).
# Run 30: push native-aspect resolution 384x165 -> 448x192 (H/W=2.33 kept). Resolution transferred before
# (proxy monotonic, baseline-independent regime change) — unlike width. Train cache is at this resolution;
# multi-scale TTA at inference also samples {384,448,512}. Cheaper alt if slow: IMG_H,IMG_W = 416,179.
IMG_H, IMG_W = 448, 192
...
# Run 30 FAST: 5-WAY split but train only the first TRAIN_FOLDS=2 -> each model sees 80% data (vs KFold-2's
# 50%), only 2 models. Aggressive pseudo: top-500/class, floor 0.50 (more transductive labels than Run 28).
N_SPLITS = 5; TRAIN_FOLDS = int(os.environ.get('TRAIN_FOLDS', '2')); TOPK = 500; CONF_FLOOR = 0.50; PW = 0.5
...
print(f'\nFINAL CV {best_cv:.2f}%  (Kaggle est ~{best_cv+4.0:.1f}%, native offset +4) -> best round applied', flush=True)
print('NOTE: 2-fold CV is noisier & ~0.5pp lower than 3-fold (less data/model); the multi-scale TTA gain shows up at SUBMISSION, not in this CV.', flush=True)

# ----------------------------- inference: ensemble + MULTI-SCALE + translation TTA -> submission.csv -----------------------------
# Run 30 free multi-res ensemble: EffNet's global-pool head accepts any input size, so re-scale each test
# image to {384,448,512} (native aspect kept) and average. Scales bracket the 448 train res by +/-15%.
SCALES = [(384, 165), (448, 192), (512, 220)]
...
for st in fold_states:
    ...
    with torch.no_grad(), torch.autocast(device_type=device.type, dtype=torch.float16, enabled=use_amp):
        for s0 in range(0, len(test_df), 256):
            xr = XTE[s0:s0 + 256].to(device).float() / 255.            # [B,3,448,192] in [0,1]
            for sH, sW in SCALES:
                ...
                for fs, ts in TTA:
                    ...
                    probs[s0:s0 + 256] += torch.softmax(m(x).float(), 1).cpu()
print(f'inference: {len(fold_states)} folds x {len(SCALES)} scales x {len(TTA)} TTA = {len(fold_states)*len(SCALES)*len(TTA)} passes/image', flush=True)
